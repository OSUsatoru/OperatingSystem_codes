...
/*
    ...
    this funciton to encript string
**********************************************************/
void childProcess(int connectionSocket)
{
    ...
    fflush(stdout);
    */
    /* to encript text stirng using key stirng */
    to_encrypt();
    /* send encripted stirng to client */
...
/*
    This function encript text stirng by using key string
***************************************************************/
void to_encrypt()
{
    int text_size = strlen(text_buffer);
    encrypted_buffer = (char *)calloc(text_size+1, sizeof(char));
    memset(encrypted_buffer, '\0', text_size+1);
    ...
    for(int i = 0; i < text_size; ++i){
        ...
        encrypted_buffer[i] = key_characters[(text_index + key_index) % strlen(key_characters)];
    }
    /*
    fprintf(stdout, "encripted: %s\n", encrypted_buffer);
    fflush(stdout);
    */
}
/*
...
void send_result(int connectionSocket)
{
    int enc_size = strlen(encrypted_buffer);
    /* send the size of encripted string */
    if(send(connectionSocket, &enc_size, sizeof(enc_size), 0) < 0){
        fprintf(stderr, "Server: Error Faild to send an initial message to Client\n");
        exit(2);
    }
    /*
    ...
    fflush(stdout);
    */
    /* send the encripted string */
    if(send(connectionSocket, encrypted_buffer, enc_size, 0) < 0){
        fprintf(stderr, "Server: Error Faild to send an initial message to Client\n");
        exit(2);
    }
}
