...
fn main() {
    ...
    for i in 0..2{
        // clone xs[0] and xs[1]
        xs_tmp = xs[i].clone();
        // create 2 threads, and run the function
        handles.push( thread::spawn(move|| map_data(&xs_tmp)).join().unwrap());
        intermediate_sums.push(handles[i]);
    }
    /*
    ...
    for i in 0..2{
        intermediate_sums.push(results[i]);
    }*/
    /*
    ...
    */
    ...
    let v2 = generate_data(num_elements);
    let xs2 = partition_data(num_partitions, &v2);
    // Print info about the partitions
    ...
    for i in 0..num_partitions{
        xs_tmp2 = xs2[i].clone();
        handles2.push( thread::spawn(move|| map_data(&xs_tmp2)).join().unwrap());
        intermediate_sums2.push(handles2[i]);
    }
    // Print the vector with the intermediate sums2
...
fn partition_data(num_partitions: usize, v: &Vec<usize>) -> Vec<Vec<usize>>{
    ...
    /* take a mod */
    let mut rest = v.len() % num_partitions;
    // Create a vector that will contain vectors of integers
    let mut xs: Vec<Vec<usize>> = Vec::new();
    let mut counter = 0;

    // Create num_partisions of vectors of integers
    for _i in 0..num_partitions{
        /* create a vector of integers */
        let mut x : Vec<usize> = Vec::new();
        // Add the the integers in the input vector to x
        for _j in 0..partition_size{
            /*(i*partition_size)+j*/
            x.push(v[counter]);
            counter += 1;
        }
        // if there is rest number
        if rest != 0{
            x.push(v[counter]);
            counter+=1;
            rest-=1;
